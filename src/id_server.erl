...
get_id(Node) ->
  gen_server:call({id_server, Node}, id).
...
-spec(init(Args :: term()) ->
  {ok, State :: #id_server_state{}} | {ok, State :: #id_server_state{}, timeout() | hibernate} |
...
-spec(terminate(Reason :: (normal | shutdown | {shutdown, term()} | term()),
    State :: #id_server_state{}) -> term()).
terminate(_Reason, _State = #id_server_state{}) ->
  ok.

%% @private
%% @doc Convert process state when code is changed
-spec(code_change(OldVsn :: term() | {down, term()}, State :: #id_server_state{},
    Extra :: term()) ->
  {ok, NewState :: #id_server_state{}} | {error, Reason :: term()}).
