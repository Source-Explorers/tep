...
-spec(init(Args :: term()) ->
  {ok, State :: #configuration_server_state{}} | {ok, State :: #configuration_server_state{}, timeout() | hibernate} |
...
-spec(handle_call(Request :: term(), From :: {pid(), Tag :: term()},
  ...
  {reply, Reply :: term(), NewState :: #configuration_server_state{}, timeout() | hibernate} |
...
-spec(terminate(Reason :: (normal | shutdown | {shutdown, term()} | term()),
    State :: #configuration_server_state{}) -> term()).
terminate(_Reason, _State = #configuration_server_state{}) ->
  ok.

%% @private
%% @doc Convert process state when code is changed
-spec(code_change(OldVsn :: term() | {down, term()}, State :: #configuration_server_state{},
    Extra :: term()) ->
...
config_file_name() ->
  PossibleFileNames = [ filename:absname_join(Path, File) || Path<-default_directories(), File<-?FILE_NAMES],
  ConfigFiles = [ File || File <- PossibleFileNames, filelib:is_regular(File)],
  Result = if
    length(ConfigFiles) >= 1 -> [ConfigFile | _Rest] = ConfigFiles, ConfigFile;
    true -> {error, {no_file, "No config file found"}}
  end,
  Result.